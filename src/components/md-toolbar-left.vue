<template>
    <div class="v-left-item">
        ...
        <div :disabled="!editable" :class="{'selected': s_img_dropdown_open}" type="button" v-if="toolbars.imagelink"
             ...
             aria-hidden="true">
            <transition name="fade">
                <div class="op-image popup-dropdown" :class="{'transition': transition}" v-show="s_img_dropdown_open"
                     @mouseleave="$mouseleave_img_dropdown" @mouseenter="$mouseenter_img_dropdown">
...
export default {
    ...
    mounted() {
        document.body.appendChild(this.$refs.addImageLink);
    },
    beforeDestroy() {
        document.body.removeChild(this.$refs.addImageLink);
    },
    methods: {
        $imgLinkAdd() {
