<template>
   <div :class="[{ 'fullscreen': s_fullScreen, 'shadow': boxShadow }]" :style="{'box-shadow': boxShadow ? boxShadowStyle : ''}"
        class="v-note-wrapper markdown-body">
      <!--工具栏-->
      <div v-show="toolbarsFlag" :style="{'background': toolbarsBackground}" class="v-note-op">
         <v-md-toolbar-left ref="toolbar_left" :class="{'transition': transition}" :d_words="d_words" :editable="editable"
                            :image_filter="imageFilter" :toolbars="toolbars"
                            :transition="transition"
                            @imgAdd="$imgAdd" @imgDel="$imgDel" @imgTouch="$imgTouch" @toolbar_left_addlink="toolbar_left_addlink"
         ...
         <v-md-toolbar-right ref="toolbar_right" :class="{'transition': transition}" :d_words="d_words"
                             :s_fullScreen="s_fullScreen"
                             :s_html_code="s_html_code"
                             :s_navigation="s_navigation" :s_preview_switch="s_preview_switch"
      ...
      <div class="v-note-panel">
         <!--编辑区-->
         <div ref="vNoteEdit" :class="{'scroll-style': s_scrollStyle, 'scroll-style-border-radius': s_scrollStyle && !s_preview_switch && !s_html_code, 'single-edit': !s_preview_switch && !s_html_code, 'single-show': (!s_subfield && s_preview_switch) || (!s_subfield && s_html_code), 'transition': transition}" class="v-note-edit divarea-wrapper"
              @click="textAreaFocus"
              @scroll="$v_edit_scroll">
            <div :style="{'background-color': editorBackground}" class="content-input-wrapper">
               <!-- 双栏 -->
               <v-autoTextarea ref="vNoteTextarea" v-model="d_value"
                               :fontSize="fontSize" :placeholder="placeholder ? placeholder : d_words.start_editor"
                               :style="{'background-color': editorBackground}" class="content-input" fullHeight
                               lineHeight="1.5"></v-autoTextarea>
            </div>
         </div>
         <!--展示区-->
         <div v-show="s_preview_switch || s_html_code"
              :class="{'single-show': (!s_subfield && s_preview_switch) || (!s_subfield && s_html_code)}" class="v-note-show">

            <div v-if="useVueRender" v-show="!s_html_code" ref="vShowContent"
            ...
            <div v-if="!useVueRender" v-show="!s_html_code" ref="vShowContent" :class="{'scroll-style': s_scrollStyle, 'scroll-style-border-radius': s_scrollStyle}"
                 :style="{'background-color': previewBackground}"
         ...
         <transition name="slideTop">
            <div v-show="s_navigation" :class="{'transition': transition}" class="v-note-navigation-wrapper">
               <div class="v-note-navigation-title">
                  {{ d_words.navigation_title }}<i aria-hidden="true"
                                                   class="fa fa-mavon-times v-note-navigation-close"
                                                   @click="toolbar_right_click('navigation')"></i>
               </div>
               <div ref="navigationContent" :class="{'scroll-style': s_scrollStyle}"
      ...
      <transition name="fade">
         <div ref="help">
            <div v-if="s_help" class="v-note-help-wrapper" @click="toolbar_right_click('help')">
               <div :class="{'shadow': boxShadow}" class="v-note-help-content markdown-body">
                  <i aria-hidden="true" class="fa fa-mavon-times"
                     @click.stop.prevent="toolbar_right_click('help')"></i>
